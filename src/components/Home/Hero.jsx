...
const Hero = () => {
  ...
  useEffect(() => {
    let tl = gsap.timeline({
      defaults: { ease: Power1.easeInOut, duration: 0.2 },
    });
    tl.to(".firstLayer", { y: "-100%", duration: 1, ease: "linear" })
      .to(`.${styles.banner} h1 > span`, { y: 0, duration: 0.5 })
      .to(`.${styles.banner} h4 > span`, { y: 0, duration: 0.5 })
      .to([`.${styles.banner} .btn`, "nav ul li"], { opacity: 1, stagger: 0.1 })
      ...
      .to(`.${styles.stat_seprator}`, { y: 1, stagger: 0.2, opacity: 1 })
      .to(`.${styles.stat}`, { stagger: 0.2, opacity: 1 });
  }, []);
  return (
    <section className={styles.banner}>
      {!isMobile && <div id="circle"></div>}
      ...
      <div className={styles.container}>
        ...
        <div className={styles.header_tagline}>
          ...
          <span className={styles.header_tagline_slogan + "  text-animation"}>
            {" "}
            <span>
              {" "}
              No 1 <span className={styles.colored}>Web Development</span>{" "}
              Agency <br />
        ...
        <h1 className={"heading " + styles.landingbannerheading}>
          <span>
            ...
            <span className={rightColored && styles.colored}>{heading[1]}</span>
          </span>
        </h1>
        <h4 className="mb1  mt1">
          <span>
            {subHeading[0]}{" "}
            <span className={styles.colored}> {subHeading[1]} </span>{" "}
            {subHeading[2]}{" "}
            <span className={styles.colored}> {subHeading[3]} </span>
          </span>
        </h4>
        <div className={styles.banner_btns}>
          <Link to="projects">
              <button className="btn btnPrimaryOutlined btnLg  mr1 no-circle">
                {btn1.text}{" "}
              </button>
          </Link>

          <Link to="/contact-us/#contact_form_section">
              <button className="btn btnPrimary btnLg   no-circle">
                {btn2.text}{" "}
              </button>
          </Link>
        </div>
        <div className={styles.statistics}>
          <div className={styles.stat + " big-circle"}>
            <span
              className={"heading " + styles.stat_number + "  pointerEventNone"}
            >
              100+
            </span>
            <span className={styles.stat_title + " pointerEventNone"}>
              {isMobile ? "Projects" : "Projects Done"}
            </span>
          </div>
          ...
          <div className={styles.stat + " big-circle"}>
            ...
            <span className={`${styles.stat_title}   pointerEventNone`}>
              {isMobile ? "Services" : "Services Providing"}
            </span>
          </div>
          <span className={styles.stat_seprator + " pointerEventNone"}></span>
          <div className={styles.stat + " big-circle"}>
            ...
            <span className={styles.stat_title + " pointerEventNone"}>
              {isMobile ? "Years" : "Years Experience"}
            </span>
          </div>
        </div>
      </div>
